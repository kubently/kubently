...
class AuthenticationService(Protocol):
    ...
    async def authenticate(self, api_key: str | None, authorization: str | None) -> AuthResult:
        """
...
class DefaultAuthenticationService:
    ...
    async def authenticate(self, api_key: str | None, authorization: str | None) -> AuthResult:
        ...
        if ok:
            ...
            return AuthResult(
                ok=True, identity=identity, method=method, error=None, permissions=permissions
            )
        else:
            return AuthResult(ok=False, identity=None, method=None, error="Invalid credentials")
